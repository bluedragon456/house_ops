...
SUPPORTED_EQUIPMENT: tuple[EquipmentDefinition, ...] = (
    ...
    EquipmentDefinition(
        key=EQUIPMENT_TYPE_ROBOT_VACUUM,
        label="Robot vacuum",
        description="Track robot vacuum, mop, and dock maintenance with robot-specific maintenance tasks.",
        category="Smart cleaning",
        tier=CATALOG_TIER_ADVANCED,
        applicable_dwellings=ALL_DWELLINGS,
        applicable_ownership=(OWNERSHIP_TYPE_OWNER, OWNERSHIP_TYPE_RENTER),
        primary_task_key=TASK_FILTER,
        supported_power_types=(POWER_TYPE_BATTERY,),
        default_power_type=POWER_TYPE_BATTERY,
        tasks=(
            TaskDefinition(key=TASK_FILTER, title="Filter cleaning", default_interval_days=14),
            TaskDefinition(key=TASK_DUST_BIN, title="Dust bin emptying", default_interval_days=3),
            TaskDefinition(key=TASK_MAIN_BRUSH, title="Main brush cleaning", default_interval_days=14),
            TaskDefinition(key=TASK_SIDE_BRUSH, title="Side brush cleaning", default_interval_days=14),
            TaskDefinition(key=TASK_WHEEL_CLEAN, title="Wheel and hair cleanup", default_interval_days=14),
            TaskDefinition(key=TASK_SENSOR_CLEANING, title="Sensor cleaning", default_interval_days=30),
            TaskDefinition(key=TASK_CONTACT_CLEANING, title="Charging contact cleaning", default_interval_days=30),
            TaskDefinition(key=TASK_MOP_SERVICE, title="Mop pad / roller cleaning", default_interval_days=7, optional=True),
            TaskDefinition(key=TASK_WATER_TANK_CLEANING, title="Water tank cleaning", default_interval_days=14, optional=True),
            TaskDefinition(key=TASK_DOCK_DUST_BAG, title="Dock dust bag replacement", default_interval_days=60, optional=True),
            TaskDefinition(key=TASK_DOCK_AIR_PATH, title="Dock dust path cleaning", default_interval_days=30, optional=True),
            TaskDefinition(key=TASK_DOCK_CLEAN_WATER_TANK, title="Dock clean water tank cleaning", default_interval_days=30, optional=True),
            TaskDefinition(key=TASK_DOCK_DIRTY_WATER_TANK, title="Dock dirty water tank cleaning", default_interval_days=7, optional=True),
            TaskDefinition(key=TASK_DOCK_WASH_TRAY, title="Dock wash tray cleaning", default_interval_days=14, optional=True),
            TaskDefinition(key=TASK_DOCK_WATER_FILTER, title="Dock water filter cleaning", default_interval_days=30, optional=True),
        ),
    ),
